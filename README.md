# @p2plabs/peersky-chrome-extensions

> Chrome extension API support for Electron, built for Peersky.

Npm: [`@p2plabs/peersky-chrome-extensions`](https://www.npmjs.com/package/@p2plabs/peersky-chrome-extensions).
...
1. Include `node_modules` in your packaged app. This allows `@p2plabs/peersky-chrome-extensions/preload` to
   be resolved.
...
| [`chrome.extension`](https://developer.chrome.com/docs/extensions/reference/extension/) | Best-effort `getViews`: always returns plain metadata objects (`id`, `type`, `windowId`, `tabId`, `url`). In a popup with `fetchProperties.type === 'popup'`, entries that represent the current page include `self: true` when the IPC list omits or matches by URL — not live `Window` instances. |
